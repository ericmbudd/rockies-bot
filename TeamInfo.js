...
function pullFullSchedule() {
let dateTime = new Date();
  let timezone = Session.getScriptTimeZone();  
  let year = Utilities.formatDate(dateTime, timezone, "yyyy")

  url = 'https://statsapi.mlb.com/api/v1/schedule?sportId=1&teamId=115&startDate=' + year + '-02-01&endDate=' + year + '-11-10'

  Logger.log('pull full schedule url = ' + url)
  let response = JSON.parse(UrlFetchApp.fetch(url))//.getContentText();
  scheduleRaw = response

  //scheduleRaw = getFullTestSchedule()
  //Logger.log(scheduleRaw)

  schedule = [];

  for (let d of scheduleRaw.dates) {
    for (let g of d.games) {
      if ((g.seriesDescription == 'Regular Season' || g.seriesDescription == 'Spring Training') && g.status.detailedState != 'Postponed') {
        //month = g.officialDate.substring(5,6).str.replace(/^0+(?=\d)/, '')
        

        game = {}
        game.officialDate = g.officialDate
        game.gameDate =  new Date(g.gameDate)
        game.shortDate =  g.officialDate.substring(5,7).replace(/^0+(?=\d)/, '') + '/' + g.officialDate.substring(8,10).replace(/^0+(?=\d)/, '')
        game.detailedState = g.detailedState
        game.awayTeam = g.teams.away.team.name
        game.homeTeam = g.teams.home.team.name
        game.homePct = g.teams.home.leagueRecord.pct
        game.awayPct = g.teams.away.leagueRecord.pct
        game.homeScore = g.teams.home.score
        game.awayScore = g.teams.away.score
        game.seriesGameNumber = g.seriesGameNumber
        game.gamesInSeries = g.gamesInSeries
        game.myTeam = game.awayTeam == 'Colorado Rockies' ? 'away' : 'home'
        game.opposingTeam = game.awayTeam == 'Colorado Rockies' ? 'home' : 'away'
        schedule.push(game)
      }
    }
  }
  //Logger.log(schedule.length)
  //Logger.log(schedule)

  return schedule;
}

function updateRockies40ManRoster() {
  rosterOutput = pullRockies40ManRoster()
  output40ManRoster(rosterOutput);
}

function pullRockies40ManRoster() {
  return pull40ManRoster('Colorado Rockies')
}


function pull40ManRoster(teamName, minors) {
  var _ = LodashGS.load();
  teamID = allTeamInfo()[teamName].id
  url = `https://statsapi.mlb.com/api/v1/teams/${teamID}/roster/40Man`

  let response = JSON.parse(UrlFetchApp.fetch(url))//.getContentText();
  rosterRaw = response.roster
  //rosterRaw = testRoster().roster

  rosterProcessed = _.orderBy(rosterRaw, 'status.code', 'asc')

  rosterOutput = []

  for (let p of rosterProcessed) {
    if (minors == 0 && p.status.description != 'Reassigned to Minors') {
      rosterOutput.push([teamName, p.person.fullName, p.jerseyNumber, p.position.name, p.status.description, p.note])
    }
    else if (minors != 0) {
      rosterOutput.push([teamName, p.person.fullName, p.jerseyNumber, p.position.name, p.status.description, p.note])
    }
  ...
  Logger.log(rosterOutput)
  return rosterOutput
}

function output40ManRoster(rosterOutput) {
  var sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("Roster");  
  sheet.getRange(2,1,rosterOutput.length,6).setValues(rosterOutput);
}


function outputPlayoffs40ManRoster(rosterOutput) {
  var sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("Playoff Roster");  
  sheet.getRange(2,1,rosterOutput.length,6).setValues(rosterOutput);
}
...
function nextSeriesSchedule() {
  ...
  nextGameIndex = 0;
  nextSeriesLookAhead = 0;
  nextSeriesArray = []


  for (i=0; i < schedule.length; i++) {
    gameDate = schedule[i].gameDate
    //Logger.log(gameDate)
  ...
  for (i=nextGameIndex; i < schedule.length && nextSeriesLookAhead < 3; i++ ) {
    if (schedule[i].seriesGameNumber != 1) {
      continue
    }
    nextSeriesArray.push(schedule[i])
    nextSeriesLookAhead = nextSeriesLookAhead + 1;
  }

  Logger.log(schedule[nextGameIndex].gameDate)
  Logger.log(nextGameIndex)
  Logger.log(nextSeriesArray)

  return nextSeriesArray;
}

function pullAllTeamStandings(sortByField, sortByDirection) {
  var _ = LodashGS.load();
  let dateTime = new Date();
  let timezone = Session.getScriptTimeZone();  
  let year = Utilities.formatDate(dateTime, timezone, "yyyy")
  url = 'https://statsapi.mlb.com/api/v1/standings?leagueId=104,103&season=' + year + '&standingsTypes=regularSeason&hydrate=hydrations,team,record(division)' //+ hydrations
  
  baseTeamUrl = 'https://statsapi.mlb.com'
  let response = JSON.parse(UrlFetchApp.fetch(url))//.getContentText();
  standings = response

  teamInfo = allTeamInfo()

  //standings = testDataAllTeamStandings()
  standingsProcessed = []

  for (let division of standings.records) {
    for (let value of division.teamRecords) {
      standingsProcessed.push(value)
    }
  }

  standingsProcessed = _.orderBy(standingsProcessed, sortByField, sortByDirection) //'runDifferential', 'desc'

/*
  Logger.log(teamInfo['Colorado Rockies'])

  for (let teams of standingsProcessed) {
    Logger.log(teamInfo[teams.team.name].abbreviation + ' ' +  teams.runDifferential)
  }
*/

  return standingsProcessed;

}



function pullAllTeamInfo() {
  url = 'https://statsapi.mlb.com/api/v1/standings?leagueId=104,103&season=2025&standingsTypes=regularSeason&hydrate=hydrations,team,record(division)' //+ hydrations
...
/*
  for (let divisions of standings.records) {
    for (let teams of standings.records[0].teamRecords) {
      Logger.log(teams.team.link)
  }
  }
...
teamInfo = {}
 ...
 for (let division of standings.records) {
    for (let value of division.teamRecords) {
      //Logger.log(value.team.link)
        let response = JSON.parse(UrlFetchApp.fetch(baseTeamUrl + value.team.link))//.getContentText();
        teamInfo[response.teams[0].name] = response.teams[0] 
        //Logger.log(teamInfo)
        Utilities.sleep(500)
    }
 }

  var sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("AllTeamInfo");  
  sheet.getRange(2,1,1,1).setValues([[JSON.stringify(teamInfo)]]);
}



function allTeamInfo() {
  data = 
{
  "Toronto Blue Jays": {
    "springLeague": {
